...
regularQuery : singleQuery ( SP? unionClause )* ;

unionClause : ( UNION SP ALL SP? singleQuery )
...
singlePartQuery : ( ( readingClause SP? )* returnClause )
...
readingClause : matchClause
                 | unwindClause
                 | inQueryCall
                 ;

matchClause : ( OPTIONAL SP )? MATCH SP? pattern ( SP? whereClause )? ;

OPTIONAL : ( 'O' | 'o' ) ( 'P' | 'p' ) ( 'T' | 't' ) ( 'I' | 'i' ) ( 'O' | 'o' ) ( 'N' | 'n' ) ( 'A' | 'a' ) ( 'L' | 'l' )  ;

MATCH : ( 'M' | 'm' ) ( 'A' | 'a' ) ( 'T' | 't' ) ( 'C' | 'c' ) ( 'H' | 'h' )  ;

unwindClause : UNWIND SP? expr SP AS SP variable ;
...
setItem : ( propertyExpr SP? '=' SP? expr )
           | ( variable SP? '=' SP? expr )
           | ( variable SP? '+=' SP? expr )
           | ( variable SP? nodeLabels )
           ;

deleteClause : ( DETACH SP )? DELETE SP? expr ( SP? ',' SP? expr )* ;
...
yieldItems : ( '*' | ( yieldItem ( SP? ',' SP? yieldItem )* ) ) ( SP? whereClause )? ;

yieldItem : ( procedureResultField SP AS SP )? variable ;

withClause : WITH ( SP? DISTINCT )? SP returnBody ( SP? whereClause )? ;
...
returnBody : returnItems ( SP orderClause )? ( SP skipClause )? ( SP limitClause )? ;

returnItems : ( '*' ( SP? ',' SP? returnItem )* )
               | ( returnItem ( SP? ',' SP? returnItem )* )
               ;

returnItem : ( expr SP AS SP variable )
              | expr
              ;

orderClause : ORDER SP BY SP sortItem ( ',' SP? sortItem )* ;

ORDER : ( 'O' | 'o' ) ( 'R' | 'r' ) ( 'D' | 'd' ) ( 'E' | 'e' ) ( 'R' | 'r' )  ;

BY : ( 'B' | 'b' ) ( 'Y' | 'y' )  ;

skipClause : L_SKIP SP expr ;

L_SKIP : ( 'S' | 's' ) ( 'K' | 'k' ) ( 'I' | 'i' ) ( 'P' | 'p' )  ;

limitClause : LIMIT SP expr ;

LIMIT : ( 'L' | 'l' ) ( 'I' | 'i' ) ( 'M' | 'm' ) ( 'I' | 'i' ) ( 'T' | 't' )  ;

sortItem : expr ( SP? ( ASCENDING | ASC | DESCENDING | DESC ) )? ;
...
orExpr : xorExpr ( SP OR SP xorExpr )* ;

OR : ( 'O' | 'o' ) ( 'R' | 'r' )  ;

xorExpr : andExpr ( SP XOR SP andExpr )* ;
...
comparisonExpr : addOrSubtractExpr ( SP? partialComparisonExpr )* ;

addOrSubtractExpr : multiplyDivideModuloExpr ( ( SP? '+' SP? multiplyDivideModuloExpr ) | ( SP? '-' SP? multiplyDivideModuloExpr ) )* ;

multiplyDivideModuloExpr : powerOfExpr ( ( SP? '*' SP? powerOfExpr ) | ( SP? '/' SP? powerOfExpr ) | ( SP? '%' SP? powerOfExpr ) )* ;

powerOfExpr : unaryAddOrSubtractExpr ( SP? '^' SP? unaryAddOrSubtractExpr )* ;

unaryAddOrSubtractExpr : ( ( '+' | '-' ) SP? )* stringListNullOperatorExpr ;

stringListNullOperatorExpr : propertyOrLabelsExpr ( stringOperatorExpr | listOperatorExpr | nullOperatorExpr )* ;

listOperatorExpr : ( SP IN SP? propertyOrLabelsExpr )
                          | ( SP? '[' expr ']' )
                          | ( SP? '[' expr? '..' expr? ']' )
                          ;
...
stringOperatorExpr : ( ( SP STARTS SP WITH ) | ( SP ENDS SP WITH ) | ( SP CONTAINS ) ) SP? propertyOrLabelsExpr ;
...
nullOperatorExpr : ( SP IS SP NULL )
...
propertyOrLabelsExpr : atom ( SP? propertyLookup )* ( SP? nodeLabels )? ;
...
atom : literal
        ...
        | ( ALL SP? '(' SP? filterExpr SP? ')' )
        | ( ANY SP? '(' SP? filterExpr SP? ')' )
        | ( NONE SP? '(' SP? filterExpr SP? ')' )
        | ( SINGLE SP? '(' SP? filterExpr SP? ')' )
...
listLiteral : '[' SP? ( expr SP? ( ',' SP? expr SP? )* )? ']' ;

partialComparisonExpr : ( '=' SP? addOrSubtractExpr )
                               | ( '<>' SP? addOrSubtractExpr )
                               | ( '<' SP? addOrSubtractExpr )
                               | ( '>' SP? addOrSubtractExpr )
                               | ( '<=' SP? addOrSubtractExpr )
                               | ( '>=' SP? addOrSubtractExpr )
...
parenthesizedExpr : '(' SP? expr SP? ')' ;

relationshipsPattern : nodePattern ( SP? patternElementChain )+ ;

filterExpr : idInColl ( SP? whereClause )? ;

idInColl : variable SP IN SP expr ;

functionInvocation : functionName SP? '(' SP? ( DISTINCT SP? )? ( expr SP? ( ',' SP? expr SP? )* )? ')' ;
...
explicitProcedureInvocation : procedureName SP? '(' SP? ( expr SP? ( ',' SP? expr SP? )* )? ')' ;
...
listComprehension : '[' SP? filterExpr ( SP? '|' SP? expr )? SP? ']' ;

patternComprehension : '[' SP? ( variable SP? '=' SP? )? relationshipsPattern SP? ( WHERE SP? expr SP? )? '|' SP? expr SP? ']' ;

propertyLookup : '.' SP? ( propertyKeyName ) ;

caseExpr : ( ( CASE ( SP? caseAlternatives )+ ) | ( CASE SP? expr ( SP? caseAlternatives )+ ) ) ( SP? ELSE SP? expr )? SP? END ;
...
caseAlternatives : WHEN SP? expr SP? THEN SP? expr ;
...
mapLiteral : '{' SP? ( propertyKeyName SP? ':' SP? expr SP? ( ',' SP? propertyKeyName SP? ':' SP? expr SP? )* )? '}' ;

parameter : '$' ( symbolicName | DecimalInteger ) ;

propertyExpr : atom ( SP? propertyLookup )+ ;
